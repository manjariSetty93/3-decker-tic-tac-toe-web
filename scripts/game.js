...
var State = function(old) {
    ...
    this.isTerminal = function() {
		...
		//second decker
			...
			//check diagonals
			for(var i = 0, j = 4; i <= 2 ; i = i + 2, j = j - 2) {
				if(B[i+9] !== "E" && B[i+9] == B[i + j+9] && B[i + j+9] === B[i + 2*j+9]) {
					this.result = B[i+9] + "-won"; //update the state result
					return true;
				}
			}
		...
		//third decker
			...
			//check diagonals
			for(var i = 0, j = 4; i <= 2 ; i = i + 2, j = j - 2) {
				if(B[i+18] !== "E" && B[i+18] == B[i + j+18] && B[i + j+18] === B[i + 2*j+18]) {
					this.result = B[i+18] + "-won"; //update the state result
					return true;
				}
			}
			
		//check winning conditions for cases spread across all 3 deckers
			
			//checking correspondance..ie same location on all 3 deckers
			for(var i = 0,j=9,k=18; i <= 8,j<=17,k<=26 ; i++,j++,k++) {
				if(B[i] !== "E" && B[i] === B[j] && B[j] === B[k]) {
					this.result = B[i] + "-won"; //update the state result
					return true;
				}
			}
			
			//check single decker like cases spread across all 3 deckers
					//check rows
					for(var i = 0; i <= 6; i = i + 3) {
						if(B[i] !== "E" && B[i] === B[i + 1+9] && B[i + 1+9] == B[i + 2+18]) {
							this.result = B[i] + "-won"; //update the state result
							return true;
						}
					}
					...
					//check columns
					for(var i = 0; i <= 2 ; i++) {
						if(B[i] !== "E" && B[i] === B[i + 3+9] && B[i + 3+9] === B[i + 6+18]) {
							this.result = B[i] + "-won"; //update the state result
							return true;
						}
					}
					...
					//check diagonals
					for(var i = 0, j = 4; i <= 2 ; i = i + 2, j = j - 2) {
						if(B[i] !== "E" && B[i] == B[i + j+9] && B[i + j+9] === B[i + 2*j+18]) {
							this.result = B[i] + "-won"; //update the state result
							return true;
						}
					}	
	
		//done checking for terminal states, since its not in terminal state, check if its draw	
        ...
        if(available.length == 0) {
			...
			//hasn't reached any terminal state and the board isn't full either..therefore game hasn't ended yet
            return false;
        }
    };//end of isTerminal check function
